...
@masked_loss
def wl1_loss(pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    """L1 loss.
    ...
    # print(pred.size())
    h, w = pred.shape[2:]
    weight = torch.zeros(h, w).cuda()
    for i in range(h):
        weight[i, :] = torch.cos(torch.tensor((i - h / 2 + 0.5) * (torch.pi / h)))
    wl1 = (1 / torch.sum(weight)) * torch.sum(abs(pred - target) * weight)
    return wl1
...
class WL1Loss(nn.Module):
    """WL1 (mean absolute error, MAE) loss.
    ...
    def forward(self,
        ...
        Args:
            ...
            weight (Tensor, optional): of shape (N, C, H, W). Element-wise
                weights. Default: None.
        """
        return self.loss_weight * wl1_loss(
            pred,
            target,
            weight,
            reduction=self.reduction,
